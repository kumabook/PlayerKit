...
class AppleMusicPlayer: ConcreteQueuePlayer {
    ...
    internal var state: PlayerState { return _state }
    fileprivate var _state: PlayerState {
        didSet {
            if oldValue != _state {
    ...
    func playbackStateDidChange(_ notification: Notification) {
        ...
        case .playing:     _state = .play
        case .paused:      _state = .pause
        case .stopped:     _state = .pause
        case .interrupted: _state = .pause
        default:        break
        }
    }
