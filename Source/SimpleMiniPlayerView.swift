...
public class SimpleMiniPlayerView: MiniPlayerView {
    ...
    internal func initializeSubviews() {
        ...
        playButton.addTarget(    self, action: #selector(SimpleMiniPlayerView.playButtonTapped),     forControlEvents: UIControlEvents.TouchUpInside)
        previousButton.addTarget(self, action: #selector(SimpleMiniPlayerView.previousButtonTapped), forControlEvents: UIControlEvents.TouchUpInside)
        nextButton.addTarget(    self, action: #selector(SimpleMiniPlayerView.nextButtonTapped),     forControlEvents: UIControlEvents.TouchUpInside)
        
        playButton.setImage(UIImage(named: "play", inBundle: bundle, compatibleWithTraitCollection: nil), forState: UIControlState.Normal)
        state = .Pause
    }
    
    public override func updateViewWithPlayer(player: Player?) {
        guard let player = player, track = player.currentTrack else {
            titleLabel.text    = ""
            durationLabel.text = "00:00"
            return
        }
